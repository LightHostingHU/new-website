/*
  ...
  - You are about to alter the column `vps_name` on the `service` table. The data in that column could be lost. The data in that column will be cast from `VarChar(255)` to `Int`.
  ...
  - A unique constraint covering the columns `[vps_name]` on the table `Service` will be added. If there are existing duplicate values, this will fail.
...
ALTER TABLE `service` MODIFY `buy_date` DATETIME NOT NULL,
    ...
    MODIFY `vps_name` INTEGER NULL;

-- AlterTable
ALTER TABLE `transactions` MODIFY `invoiceDate` DATETIME NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX `Service_vps_name_key` ON `Service`(`vps_name`);
